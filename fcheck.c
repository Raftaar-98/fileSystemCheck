...
int inode_chk1(char* addr)
{
    ...
    for (i = 0; i < (int)(superblock->ninodes); i++, ip++)
    {
        if (ip->nlink > 0)
        {

            if ((ip->type != T_FILE) && (ip->type != T_DIR) && (ip->type != T_DEV) && (ip->type != 0))
            {
...
int addr_chk7(char* addr)
{
    ...
    for (i = 0; i < superblock->ninodes - 1; i++)
    {
        for (j = 0; j < NDIRECT; j++)
        {
            if (ip[i].addrs[j] == 0)
...
int directory_chk12(char* addr)
{
    ...
    for (i = 0; i < superblock->ninodes; i++)
    {
        ...
        if ((ip1[i].addrs[0] == 0) && (ip1[i].nlink > 1) && (ip1[i].type == T_FILE))
        {
